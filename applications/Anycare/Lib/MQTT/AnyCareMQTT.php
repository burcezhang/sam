...
class AnyCareMQTT {
  ...
  */
  ...
  function AnyCareMQTT() {
    if ($this->debug) $this->_e('AnyCareMQTT()');

    if ($this->debug) $this->_x('AnyCareMQTT()');
  }
  ...
  /* ---------------------------------
     ...
     --------------------------------- */
  function Commit() {
    if ($this->debug) $this->_e('AnyCareMQTT.Commit()');

    $errno = 100;
    $error = 'Unsupported operation for MQTT protocol!';
    $rc = false;

    if ($this->debug) $this->_x("AnyCareMQTT.Commit() rc=$rc");
    return $rc;
  }
  ...
  function Connect($proto, $options=array()) {
    if ($this->debug) $this->_e('AnyCareMQTT.Connect()');
    ...
    if ($this->debug) $this->_t("AnyCareMQTT.Connect() host=$this->host, port=$this->port, cleanstart=$this->cleanstart");
    ...
    if ($this->debug) $this->_x("AnyCareMQTT.Connect() rc=$this->virtualConnected");
  ...
  /* ---------------------------------
     ...
     --------------------------------- */
  function Disconnect() {
    if ($this->debug) $this->_e('AnyCareMQTT.Disconnect()');
    $rc = false;
    ...
    if ($this->virtualConnected) {
        if ($this->connected) {
            ...
            if ($this->debug) $this->_t('AnyCareMQTT.Disconnect() response is '.strlen($response).' bytes');
    ...
    if ($this->debug) $this->_x("AnyCareMQTT.Disconnect() rc=$rc");
    return $rc;
  }
  ...
  function IsConnected() {
    if ($this->debug) $this->_e('AnyCareMQTT.IsConnected()');
    $rc = false;
    ...
    if ($this->debug) $this->_x("AnyCareMQTT.IsConnected() rc=$rc");
    return $rc;
  }
  ...
  /* ---------------------------------
     ...
     --------------------------------- */
  function Peek() {
    if ($this->debug) $this->_e('AnyCareMQTT.Peek()');

    $errno = 100;
    $error = 'Unsupported operation for MQTT protocol!';
    $rc = false;

    if ($this->debug) $this->_x("AnyCareMQTT.Peek() rc=$rc");
    return $rc;
  }
  ...
  /* ---------------------------------
     ...
     --------------------------------- */
  function PeekAll() {
    if ($this->debug) $this->_e('AnyCareMQTT.PeekAll()');

    $errno = 100;
    $error = 'Unsupported operation for MQTT protocol!';
    $rc = false;

    if ($this->debug) $this->_x("AnyCareMQTT.PeekAll() rc=$rc");
    return $rc;
  }
  ...
  function Receive($sub_id, $options=array()) {
    if ($this->debug) $this->_e('AnyCareMQTT.Receive()');
    $rc = false;
    ...
    if (!$this->connected) {
        if ($this->debug) $this->_t('AnyCareMQTT.Receive() Not connected.');
        ...
        if ($this->sub_id != $si) {
            if ($this->debug) $this->_t('AnyCareMQTT.Receive() Connected with wrong sub_id.');
            /* No, We better reconnect then...  */
            $this->disconnect();
            $this->sub_id = $si;
            $rc = $this->do_connect_now();
        } else {
            if ($this->debug) $this->_t('AnyCareMQTT.Receive() Connected OK.');
            $rc = true;
        }
    }
    ...
    if ($rc) {
        ...
        if ($options['SAM_WAIT'] > 1) {
            ...
            if ($this->debug) $this->_t('AnyCareMQTT.Receive() timeout='.$options['SAM_WAIT']." ($s secs $m millisecs)");
            stream_set_timeout($this->sock, $s, $m);
            if ($this->debug) $this->_t('AnyCareMQTT.Receive() timeout set.');
        } else {
            if ($this->debug) $this->_t('AnyCareMQTT.Receive() no timeout value found!');
        }
            ...
            if ($hdr['mtype'] == $this->operations['MQTT_PUBLISH']) {
                ...
                if ($len > 1) {
                    ...
                    if (!$topic) {
                        ...
                        if ($this->debug) $this->_t('AnyCareMQTT.Receive() topic='.$topic);
                        $len -= (strlen($topic) + 2);
                        ...
                        if ($hdr['qos'] > 0) {
                            ...
                            $mid = $fields['a'];
                            if ($this->debug) $this->_t('AnyCareMQTT.Receive() mid='.$mid);
                        }
                        $payload = fread($this->sock, $len);
                        if ($this->debug) $this->_t('AnyCareMQTT.Receive() payload='.$payload);
                        $rc = new MQTTMessage();
                        $rc->body = $payload;
                ...
                if ($this->debug) $this->_t('AnyCareMQTT.Receive() Receive failed response mtype = '.$mtype);
                $rc = false;
            }
        }
    }

    if ($this->debug) $this->_x("AnyCareMQTT.Receive() rc=$rc");
    return $rc;
  }
  ...
  /* ---------------------------------
     ...
     --------------------------------- */
  function Remove() {
    if ($this->debug) $this->_e('AnyCareMQTT.Remove()');

    $errno = 100;
    $error = 'Unsupported operation for MQTT protocol!';
    $rc = false;

    if ($this->debug) $this->_x("AnyCareMQTT.Remove() rc=$rc");
    return $rc;
  }
  ...
  /* ---------------------------------
     ...
     --------------------------------- */
  function Rollback() {
    if ($this->debug) $this->_e('AnyCareMQTT.Rollback()');

    $errno = 100;
    $error = 'Unsupported operation for MQTT protocol!';
    $rc = false;

    if ($this->debug) $this->_x("AnyCareMQTT.Rollback() rc=$rc");
    return $rc;
  }
  ...
  function Send($topic, $message, $options=array()) {
    if ($this->debug) $this->_e('AnyCareMQTT.Send()');
    $rc = true;
        ...
        if ($hdr) {
            ...
            if ($qos == 1) {
                ...
                if ($hdr['mtype'] == $this->operations['MQTT_PUBACK']) {
                    ...
                    if ($len < 2) {
                        if ($this->debug) $this->_t("AnyCareMQTT.Send() send failed, incorrect length response ($len) received!");
                        $this->errno = 302;
                    ...
                    if ($this->debug) $this->_t('AnyCareMQTT.Send() Send failed response mtype = '.$mtype.' Expected PUBREC!');
                    $rc = false;
                }
            } else {
                ...
                if ($hdr['mtype'] == $this->operations['MQTT_PUBREC']) {
                    ...
                    if ($len < 2) {
                        ...
                        if ($hdr['mtype'] == $this->operations['MQTT_PUBCOMP']) {
                            ...
                            if ($this->debug) $this->_t('AnyCareMQTT.Send() Send failed response mtype = '.$mtype.' Expected PUBCOMP!');
                            $rc = false;
                        }
                    }
                } else {
                    if ($this->debug) $this->_t('AnyCareMQTT.Send() Send failed response mtype = '.$mtype);
                    $rc = false;
                }
            }
        }
    ...
    if ($this->debug) $this->_x("AnyCareMQTT.Send() rc=$rc");
    return $rc;
  }
  ...
  function Subscribe($topic, $options=array()) {
    if ($this->debug) $this->_e("AnyCareMQTT.Subscribe($topic)");
    $rc = true;
    ...
    $hdr = $this->read_fixed_header($this->sock);
    if (!$hdr) {
        if ($this->debug) $this->_t("AnyCareMQTT.Subscribe() subscribe failed, no response from broker!");
        $this->errno = 301;
        ...
        if ($hdr['mtype'] == $this->operations['MQTT_SUBACK']) {
            ...
            if ($len < 3) {
                if ($this->debug) $this->_t("AnyCareMQTT.Subscribe() subscribe failed, incorrect length response ($len) received!");
                $this->errno = 301;
                $this->error = 'Subscribe request failed, incorrect length response ($len) received!';
                $rc = false;
            }
        } else {
            if ($this->debug) $this->_t('AnyCareMQTT.Subscribe() subscribe failed response mtype = '.$mtype);
            $rc = false;
        }
    }

    if ($this->debug) $this->_x("AnyCareMQTT.Subscribe() rc=$rc");
    return $rc;
  }
  ...
  function Unsubscribe($sub_id) {
    if ($this->debug) $this->_e("AnyCareMQTT.Unsubscribe($sub_id)");
    ...
    /* Are we already connected?               */
    if (!$this->connected) {
        if ($this->debug) $this->_t('AnyCareMQTT.Unsubscribe() Not connected.');
        /* No, so open up the connection...    */
        $this->sub_id = $si;
        $rc = $this->do_connect_now();
    } else {
        /* We are already connected. Are we using the right subscriber id?  */
        if ($this->sub_id != $si) {
            if ($this->debug) $this->_t('AnyCareMQTT.Unsubscribe() Connected with wrong sub_id.');
            /* No, We better reconnect then...  */
            $this->disconnect();
            $this->sub_id = $si;
            $rc = $this->do_connect_now();
        } else {
            if ($this->debug) $this->_t('AnyCareMQTT.Unsubscribe() Connected OK.');
            $rc = true;
        }
    }
    ...
    $hdr = $this->read_fixed_header($this->sock);
    if (!$hdr) {
        if ($this->debug) $this->_t("AnyCareMQTT.Unsubscribe() unsubscribe failed, no response from broker!");
        $this->errno = 302;
        ...
        if ($hdr['mtype'] == $this->operations['MQTT_UNSUBACK']) {
            ...
            if ($len != 2) {
                if ($this->debug) $this->_t("AnyCareMQTT.Unsubscribe() unsubscribe failed, incorrect length response ($len) received!");
                $this->errno = 301;
                $this->error = "Unsubscribe request failed, incorrect length response ($len) received!";
                $rc = false;
            }
        } else {
            if ($this->debug) $this->_t('AnyCareMQTT.Unsubscribe() unsubscribe failed response mtype = '.$hdr['mtype']);
            $rc = false;
        }
    }

    if ($this->debug) $this->_x("AnyCareMQTT.Unsubscribe() rc=$rc");
    return $rc;
  }
  ...
  function remaining_length($l) {
    ...
    if ($this->debug) $this->_t("AnyCareMQTT.remaining_length() l=$l");

    $rlf = '';
    do {
      $digit = $l % 128;
      $l = ($l - $digit)/128;
      if ($this->debug) $this->_t("AnyCareMQTT.remaining_length() digit=$digit l=$l");
      ...
      $rlf .= $digit;
      if ($this->debug) $this->_t("AnyCareMQTT.remaining_length() rlf=$rlf");
    } while ($l > 0);
  ...
  function checkHost($hostname, $port) {
      if ($this->debug) $this->_e("AnyCareMQTT.checkHost($hostname)");
      $rc = false;
      ...
      if ($this->debug) $this->_x("AnyCareMQTT.checkHost(rc=$rc)");
      return $rc;
  }
  ...
  function do_connect_now() {
    ...
    if ($this->sub_id == '') {
        ...
        if ($this->debug) $this->_t("AnyCareMQTT.do_connect_now() sub_id=$this->sub_id");
    } else {
        if ($this->debug) $this->_t("AnyCareMQTT.do_connect_now() using existing sub_id=$this->sub_id");
    }
    ...
    if (!$this->virtualConnected) {
        ...
        if (!$fp) {
            if ($this->debug) $this->_t("AnyCareMQTT.do_connect_now() fsockopen failed! ($errno) $errstr");
            $this->errno = 208;
    ...
    if ($hdr) {
        if ($hdr['mtype'] == $this->operations['MQTT_CONNACK']) {
            $len = $this->read_remaining_length($this->sock);
            if ($len < 2) {
                if ($this->debug) $this->_t("AnyCareMQTT.do_connect_now() connect failed, incorrect length response ($len) received!");
                $this->errno = 218;
                ...
                if ($fields['retcode'] == 0) {
                    ...
                    $this->connected = true;
                    $rc = true;
                    if ($this->debug) $this->_t('AnyCareMQTT.do_connect_now() connected OK');
                } else {
                    if ($this->debug) $this->_t('AnyCareMQTT.do_connect_now() connect failed retcode = '.$fields['retcode']);
                    $rc = false;
                    if ($fields['retcode'] == 2) {
                        $this->sub_id = '';
                        $this->errno = 279;
            ...
            if ($this->debug) $this->_t('AnyCareMQTT.do_connect_now() connect failed response mtype = '.$mtype);
            $rc = false;
        }
    }
  ...
  function read_fixed_header($conn) {
      ...
      if (strlen($response) > 0) {
          ...
          $x -= ($dup * 8);
          $mtype = $x / 16;
          if ($this->debug) $this->_t("AnyCareMQTT.read_fixed_header() mtype=$mtype, dup=$dup, qos=$qos, retain=$ret");
  ...
  function read_remaining_length($conn) {
      ...
      while (!feof($conn)) {
          ...
          $x = $fields['a'];
          if ($this->debug) $this->_t('AnyCareMQTT.read_remaining_length() byte ('.strlen($byte).') = '.$x);
          if ($x < 128) {
              $rc += $x * $m;
              break;
          } else {
      ...
      if ($this->debug) $this->_t('AnyCareMQTT.read_remaining_length() remaining length = '.$rc);
      return $rc;
  }

  function read_topic($conn) {
      if ($this->debug) $this->_e('AnyCareMQTT.read_topic()');
      $rc = false;
      while (!feof($conn)) {
          $tlen = fread($conn, 2);
          $fields = unpack('na', $tlen);
          if ($this->debug) $this->_t('AnyCareMQTT.read_topic() topic length='.$fields['a']);
          $rc = fread($conn, $fields['a']);
          break;
      }
      if ($this->debug) $this->_x("AnyCareMQTT.read_topic(rc=$rc)");
      return $rc;
  }
}
